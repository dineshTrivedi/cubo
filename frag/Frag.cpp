...
void fragment(node*, cell*, vector< int >, int*, int);
void online_fragment(node*, node*, cell*, vector< int >, int*, int,
        int*, int*, bool);
...
void multi_intersect(cell*, cell **, int);
cell* cell_of(int*, int);
cell* online_cell_of(node*, int*, int, int*);
void free_tree(node*, int*, int);
string itos(int);
...
int main(int argc, char **argv)
{
    ...
    cell_name = new int[n_dimensions];
    memset(cell_name, -1, sizeof(int) * n_dimensions);
    ...
    for (int i = 0; i < root->num_cells; i++) {
        ...
        for (int j = 0; j < root->child[i]->num_cells; j++) {
...
void read_datafile(char *filename)
{
    ...
    n_frags = (int) ceil((double) n_dimensions / frag_size);
    ...
    for (int i = 0; i < n_frags; i++) {
        ...
        for (int j = 0; j < size; j++) {
            ...
            // starts at 1.
            for (int k = 1; k < cardinality[index]; k++) {
                ...
                root->child[i]->child[j]->cells[k]->trans = new
                    int[root->child[i]->child[j]->cells[k]->capacity];
            }
    ...
    frag = new int[n_dimensions];

    // keeps track of where in each fragment the dimension is
    frag_index = new int[n_dimensions];
        ...
        for (int j = 0; j < n_dimensions; j++) {
            ...
            if (c->capacity == 0) {
                ...
                if (c->num_trans >= c->capacity) {
                    ...
                    c->trans = new int[c->capacity];
                    memcpy(c->trans, old, sizeof(int) * old_size);
    ...
    for (int i = 0; i < root->num_cells; i++) {
        ...
        for (int j = 0; j < root->child[i]->num_cells; j++) {
            ...
            for (int k = 0; k < root->child[i]->child[j]->num_cells; k++) {
                ...
                if (root->child[i]->child[j]->cells[k] != NULL) {
                    ...
                    if (c->capacity - c->num_trans > SHRINK) {
                        ...
                        c->trans = new int[c->num_trans];
                        memcpy(c->trans, old, sizeof(int) * c->num_trans);
            ...
            for (int j = 0; j < root->child[i]->num_cells; j++) {
                ...
                for (int k = 0; k < root->child[i]->child[j]->num_cells; k++) {
                    ...
                    if (root->child[i]->child[j]->cells[k] == NULL)
                        ...
                        cout << "(" << i << ", " << j << ", " << k << ") -> ";
                        for (int u = 0; u < root->child[i]->child[j]->cells[k]->num_trans; u++)
...
void fragment(node *my_root, cell *parent_cell, vector< int > cuboid_name,
        int *cell_name, int n_dim)
{
    int num_children, cell_offset, temp, last_name;
    cell *c;

    if (DEBUG) {
        cout << endl << "cut starting at" << endl;
        cout << "\tcuboid: ";
        for (unsigned int i = 0; i < cuboid_name.size(); i++)
            cout << cuboid_name[i] << " ";
        cout << endl;
        cout << "\tcell  : ";
        for (int i = 0; i < n_dimensions; i++) {
    ...
    if (parent_cell != NULL) {
        ...
        for (unsigned int i = 0; i < cuboid_name.size(); i++) {
            ...
            if (cell_name[cuboid_name[i]] != -1) {
                ...
                temp = 1;
                for (unsigned int j = i + 1; j < cuboid_name.size(); j++)
        ...
        for (int i = cell_offset + 1; i < cardinality[last_name] +
                cell_offset; i++) {
            ...
            if (DEBUG) {
                ...
                for (int j = 0; j < parent_cell->num_trans; j++)
                    cout << parent_cell->trans[j] << " ";
                cout << endl;
                cout << "[" << c->num_trans << "]: ";
                for (int j = 0; j < c->num_trans; j++)
                    cout << c->trans[j] << " ";
                cout << endl;
                cout << "result: ";
...
void intersect(cell *target, cell *a, cell *b)
{
    ...
    // intersect
    for (int i = 0; i < small->num_trans; i++) {

        // look-ahead heuristic
        if (j + LOOK < big->num_trans &&
                big->trans[j + LOOK] < small->trans[i])
            j += LOOK;

        while (big->trans[j] < small->trans[i] && j < big->num_trans)
            j++;

        // found a match
        if (small->trans[i] == big->trans[j]) {

            // insert it into intersection set
            target->trans[target->num_trans] = small->trans[i];
            target->num_trans++;
    ...
    if (target->capacity - target->num_trans > SHRINK) {
        ...
        int *old = target->trans;

        // resize and copy over the older stuff
        target->capacity = target->num_trans;
        target->trans = new int[target->num_trans];
        memcpy(target->trans, old, sizeof(int) * target->num_trans);
...
void multi_intersect(cell *target, cell **sources, int num_sources)
{
    int *index = new int[num_sources];
    int size = n_tuples + 1;
    int smallest_source = -1;
    bool all_equal, done;
    ...
    memset(index, 0, sizeof(int) * num_sources);
    done = false;
...
cell* cell_of(int *cell_name, int name_size)
{
    int j, offset, temp;
    bool at_root = true;
    node *n;
        ...
        if (j == name_size) {
            ...
            for (int u = 0; u < n_dimensions; u++) {
                ...
                if (cell_name[u] > 0) {
                    ...
                    for (int v = u + 1; v < n_dimensions; v++) {
...
cell* online_cell_of(node *local_root, int *cell_name, int name_size,
        int *local_name)
{
    int j, offset, temp;
    node *n = local_root;
...
void interface()
{
    ...
    while (true) {
        ...
        // experiments
            ...
            int *query_value = new int[NUM_DIMS];
            int num_ins, num_inq;
            int r;
            int value;
            string que;
            ...
            for (int i = 0; i < NUM_QUERIES; i++) {
                ...
                while (num_ins < NUM_INS) {
                    ...
                    r = (int) ((double) NUM_DIMS * rand() / RAND_MAX);
                    if (r == NUM_DIMS) r = 0;

                    if (query_value[r] == -2) {
                        value = (int) ((double) CARD * rand() / RAND_MAX);
                        if (value == 0) value = 10;
                        query_value[r] = value;
                        num_ins++;
                    }
                ...
                int _CUBEIO_TEMP = 951483 * 8 / 1024;
                bool stop = false;
...
void execute_query(string query, bool verbose, bool file_out)
{
    ...
    n_dims = 0;
    query_cell = new int[n_dimensions];
    ...
    while (iss >> word) {
        if (word == "*")
            ...
            query_cell[n_dims] = std::stoi(word);
    ...
    int *cell_name, cell_size, num_variables, num_instantiated;
    ...
    node *local_root;
    cell *c, *temp;
    int *local_name, *local_order, index;
    vector< int > cuboid_name;
    ...
    local_name = new int[n_dimensions];
    local_order = new int[num_variables];
    ...
    index = 0;
    memset(local_name, -1, sizeof(int) * n_dimensions);
        ...
        if (query_cell[i] == -2) {
            ...
            for (int k = 1; k < cardinality[i]; k++) {
                ...
                if (num_variables == n_dimensions ||
                    ...
                    c->trans = new int[c->capacity];
                    memcpy(c->trans, temp->trans, sizeof(int) * c->capacity);
                }
    ...
    */
    ...
    for (int i = 0; i < local_root->num_cells; i++) {
...
void online_fragment(node *local_root, node *my_root, cell *parent_cell,
        vector< int > cuboid_name, int *cell_name, int n_dim, int
        *local_name, int *local_order, bool file_out)
{
    int num_children, cell_offset, temp, last_name;
    cell *c;
    ...
    if (parent_cell != NULL) {
        ...
        if (my_root->num_cells <= 0) {
            ...
            for (int i = 0; i < my_root->num_cells; i++)
        ...
        int same_frag, *same_frag_cell_name, j, cell_size;
        ...
        if (n_dim == n_dimensions) {
            ...
            if (same_frag > 0) {
                ...
                if ((int) cuboid_name.size() - same_frag - 1 > 0) {
            ...
            if (n_dim == n_dimensions && same_frag > 0) {
                ...
                if (unsame_cell == NULL) {
                    ...
                    if (local_root->cells[0]->num_trans == n_tuples) {
                        ...
                        my_root->cells[i]->trans = new int[same_cell->num_trans];
                        memcpy(my_root->cells[i]->trans,
                                same_cell->trans, sizeof(int) *
                                same_cell->num_trans);
                    }
                    else {
                        if (ONLINE_DEBUG) {
...
void free_tree(node *n, int *local_name, int n_dim)
{
    if (n == NULL)
        return;

    int num_children = n_dim - 1 - local_name[n->name];
